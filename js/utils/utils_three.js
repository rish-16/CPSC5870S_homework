...
export class ThreeEngine {
    constructor(scene, camera, renderer, controls, stats) {
        ...
        for (let i = 0; i < 500; i++) {
            let line = spawn_line(this.scene, convert_z_up_array_to_y_up_array([0,0,0]), convert_z_up_array_to_y_up_array([0,-1,0]), undefined, 1.0);
            line.visible = false;
            this.line_objects.push(line);
        }
        this.sphere_objects = [];
        for (let i = 0; i < 500; i++) {
            let sphere_object = spawn_sphere(scene, [0.,0.,0.], 1.0);
        ...
        this.cone_objects = [];
        for (let i = 0; i < 500; i++) {
            let cone_object = spawn_cone(scene, 1, 1, 10, 1);
    ...
    static #new_default_generic(orthographic_camera=false) {
        ...
        spawn_line(scene, convert_z_up_array_to_y_up_array([0.,0.,0.]), convert_z_up_array_to_y_up_array([10.,0.,0.]), false, 0.02, 0xff0000);
        spawn_line(scene, convert_z_up_array_to_y_up_array([0.,0.,0.]), convert_z_up_array_to_y_up_array([0.,10.,0.]), false, 0.02, 0x00ff00);
    ...
    draw_debug_line(start_point, end_point, render_through_other_objects=false, width=0.01, color=0x0000ff) {
    ...
    draw_debug_sphere(center_point, radius, color=0x00ff00) {
        ...
        if (this.num_sphere_objects_used_on_curr_frame >= this.sphere_objects.length) {
            for (let i = 0; i < 500; i++) {
                let sphere_object = spawn_sphere(this.scene, [0.,0.,0.], 1.0);
                sphere_object.visible = false;
                this.sphere_objects.push(sphere_object);
            }
        }
    ...
    draw_debug_cone(start_point, end_point, radius, color=0x0000ff) {
        ...
        if (this.num_cone_objects_used_on_curr_frame >= this.cone_objects.length) {
            for (let i = 0; i < 500; i++) {
                let cone_object = spawn_cone(this.scene, 1, 1, 10, 1);
                cone_object.visible = false;
                this.cone_objects.push(cone_object);
            }
        }
    ...
    draw_debug_vector(start_point, end_point, tail_width = 0.01, arrow_height = 0.15, color=0x0000ff) {
        ...
        let f = b.add(c.clone().multiplyScalar(ah/15));
        let g = convert_threejs_vector3_to_array(f);
        this.draw_debug_line(start_point, g, undefined, tail_width, color);
        this.draw_debug_cone(e, end_point, tail_width*3, color);
    }
...
export function spawn_line(scene, start_point, end_point, render_through_other_objects=false, width=0.01, color=0x0000ff) {
...
export function spawn_sphere(scene, center_point, radius, num_segments=10, color=0x00ff00) {
...
export function spawn_vector(scene, direction, start_point, arrow_length, head_length, head_width, color=0x000000) {
    let dd = new THREE.Vector3(direction[0], direction[1], direction[2]);
    dd.normalize();
    let ss = new THREE.Vector3(start_point[0], start_point[1], start_point[2]);
    let arrow_helper = new THREE.ArrowHelper(dd, ss, arrow_length, color, head_length, head_width);
    scene.add(arrow_helper);
}

export function spawn_cone(scene, radius, height, radial_segments, height_segments, color=0x0000ff) {
...
export function reset_displays(gui) {
